...
string SMLA_DIO::standard_name = "SMLA_DIO";

map<string, enum SMLA_DIO::Org> SMLA_DIO::org_map = {
    {"SMLA_DIO_1Gb", SMLA_DIO::Org::SMLA_DIO_1Gb},
    {"SMLA_DIO_2Gb", SMLA_DIO::Org::SMLA_DIO_2Gb},
    {"SMLA_DIO_4Gb", SMLA_DIO::Org::SMLA_DIO_4Gb},
    {"SMLA_DIO_8Gb", SMLA_DIO::Org::SMLA_DIO_8Gb},
};

map<string, enum SMLA_DIO::Speed> SMLA_DIO::speed_map = {
    {"SMLA_DIO_200", SMLA_DIO::Speed::SMLA_DIO_200}, 
    {"SMLA_DIO_266", SMLA_DIO::Speed::SMLA_DIO_266},
};

SMLA_DIO::SMLA_DIO(Org org, Speed speed) : 
    ...
    read_latency(speed_entry.nCL + speed_entry.nDQSCK + speed_entry.nBL)
{
    ...
    init_lambda();
    init_timing();
		read_latency_smla[0] = speed_entry.nBL;
		read_latency_smla[1] = speed_entry.nBL;
		read_latency_smla[2] = speed_entry.nBL;
		read_latency_smla[3] = speed_entry.nBL;
}

SMLA_DIO::SMLA_DIO(const string& org_str, const string& speed_str) :
    SMLA_DIO(org_map[org_str], speed_map[speed_str]) 
{
}

void SMLA_DIO::set_channel_number(int channel) {
...
void SMLA_DIO::set_rank_number(int rank) {
  assert((rank == 1) && "SMLA_DIO rank number is fixed to 1.");
  org_entry.count[int(Level::Rank)] = rank;
}

void SMLA_DIO::init_speed()
{
...
void SMLA_DIO::init_prereq()
{
    // RD
    prereq[int(Level::Rank)][int(Command::RD)] = [] (DRAM<SMLA_DIO>* node, Command cmd, int id) {
    ...
    prereq[int(Level::Bank)][int(Command::RD)] = [] (DRAM<SMLA_DIO>* node, Command cmd, int id) {
        switch (int(node->state)) {
    ...
    // REF
    prereq[int(Level::Rank)][int(Command::REF)] = [] (DRAM<SMLA_DIO>* node, Command cmd, int id) {
    ...
    // PD
    prereq[int(Level::Rank)][int(Command::PD)] = [] (DRAM<SMLA_DIO>* node, Command cmd, int id) {
        switch (int(node->state)) {
    ...
    // SR
    prereq[int(Level::Rank)][int(Command::SREF)] = [] (DRAM<SMLA_DIO>* node, Command cmd, int id) {
        switch (int(node->state)) {
...
void SMLA_DIO::init_rowhit()
{
    // RD
    rowhit[int(Level::Bank)][int(Command::RD)] = [] (DRAM<SMLA_DIO>* node, Command cmd, int id) {
        switch (int(node->state)) {
...
void SMLA_DIO::init_rowopen()
{
    // RD
    rowopen[int(Level::Bank)][int(Command::RD)] = [] (DRAM<SMLA_DIO>* node, Command cmd, int id) {
        switch (int(node->state)) {
            case int(State::Closed): return false;
...
void SMLA_DIO::init_lambda()
{
    lambda[int(Level::Bank)][int(Command::ACT)] = [] (DRAM<SMLA_DIO>* node, int id) {
        node->state = State::Opened;
        node->row_state[id] = State::Opened;};
    lambda[int(Level::Bank)][int(Command::PRE)] = [] (DRAM<SMLA_DIO>* node, int id) {
        node->state = State::Closed;
        node->row_state.clear();};
    lambda[int(Level::Rank)][int(Command::PRA)] = [] (DRAM<SMLA_DIO>* node, int id) {
        for (auto bank : node->children) {
            bank->state = State::Closed;
            bank->row_state.clear();}};
    lambda[int(Level::Rank)][int(Command::REF)] = [] (DRAM<SMLA_DIO>* node, int id) {};
    lambda[int(Level::Bank)][int(Command::RD)] = [] (DRAM<SMLA_DIO>* node, int id) {};
    lambda[int(Level::Bank)][int(Command::WR)] = [] (DRAM<SMLA_DIO>* node, int id) {};
    lambda[int(Level::Bank)][int(Command::RDA)] = [] (DRAM<SMLA_DIO>* node, int id) {
        node->state = State::Closed;
        node->row_state.clear();};
    lambda[int(Level::Bank)][int(Command::WRA)] = [] (DRAM<SMLA_DIO>* node, int id) {
        node->state = State::Closed;
        node->row_state.clear();};
    lambda[int(Level::Rank)][int(Command::PD)] = [] (DRAM<SMLA_DIO>* node, int id) {
        for (auto bank : node->children) {
            if (bank->state == State::Closed)
                continue;
    ...
    lambda[int(Level::Rank)][int(Command::PDX)] = [] (DRAM<SMLA_DIO>* node, int id) {
        node->state = State::PowerUp;};
    lambda[int(Level::Rank)][int(Command::SREF)] = [] (DRAM<SMLA_DIO>* node, int id) {
        node->state = State::SelfRefresh;};
    lambda[int(Level::Rank)][int(Command::SREFX)] = [] (DRAM<SMLA_DIO>* node, int id) {
        node->state = State::PowerUp;};
}


void SMLA_DIO::init_timing()
{
