...
string SMLA_CIO::standard_name = "SMLA_CIO";

map<string, enum SMLA_CIO::Org> SMLA_CIO::org_map = {
    {"SMLA_CIO_1Gb", SMLA_CIO::Org::SMLA_CIO_1Gb},
    {"SMLA_CIO_2Gb", SMLA_CIO::Org::SMLA_CIO_2Gb},
    {"SMLA_CIO_4Gb", SMLA_CIO::Org::SMLA_CIO_4Gb},
    {"SMLA_CIO_8Gb", SMLA_CIO::Org::SMLA_CIO_8Gb},
};

map<string, enum SMLA_CIO::Speed> SMLA_CIO::speed_map = {
    {"SMLA_CIO_200", SMLA_CIO::Speed::SMLA_CIO_200}, 
    {"SMLA_CIO_266", SMLA_CIO::Speed::SMLA_CIO_266},
};

SMLA_CIO::SMLA_CIO(Org org, Speed speed) : 
    ...
    read_latency(speed_entry.nCL + speed_entry.nDQSCK + speed_entry.nBL)
{
    ...
    init_lambda();
    init_timing();
		read_latency_smla[0] = speed_entry.nBL - 6;
		read_latency_smla[1] = speed_entry.nBL - 4;
		read_latency_smla[2] = speed_entry.nBL - 2;
		read_latency_smla[3] = speed_entry.nBL;
}

SMLA_CIO::SMLA_CIO(const string& org_str, const string& speed_str) :
    SMLA_CIO(org_map[org_str], speed_map[speed_str]) 
{
}

void SMLA_CIO::set_channel_number(int channel) {
  assert((channel == 16) && "The SMLA interface supports 4 physical and 16 logical channels.");
  org_entry.count[int(Level::Channel)] = channel;
}

void SMLA_CIO::set_rank_number(int rank) {
  assert((rank == 1) && "SMLA_CIO rank number is fixed to 1.");
  org_entry.count[int(Level::Rank)] = rank;
}

void SMLA_CIO::init_speed()
{
...
void SMLA_CIO::init_prereq()
{
    // RD
    prereq[int(Level::Rank)][int(Command::RD)] = [] (DRAM<SMLA_CIO>* node, Command cmd, int id) {
    ...
    prereq[int(Level::Bank)][int(Command::RD)] = [] (DRAM<SMLA_CIO>* node, Command cmd, int id) {
        switch (int(node->state)) {
    ...
    // REF
    prereq[int(Level::Rank)][int(Command::REF)] = [] (DRAM<SMLA_CIO>* node, Command cmd, int id) {
    ...
    // PD
    prereq[int(Level::Rank)][int(Command::PD)] = [] (DRAM<SMLA_CIO>* node, Command cmd, int id) {
        switch (int(node->state)) {
    ...
    // SR
    prereq[int(Level::Rank)][int(Command::SREF)] = [] (DRAM<SMLA_CIO>* node, Command cmd, int id) {
        switch (int(node->state)) {
...
void SMLA_CIO::init_rowhit()
{
    // RD
    rowhit[int(Level::Bank)][int(Command::RD)] = [] (DRAM<SMLA_CIO>* node, Command cmd, int id) {
        switch (int(node->state)) {
...
void SMLA_CIO::init_rowopen()
{
    // RD
    rowopen[int(Level::Bank)][int(Command::RD)] = [] (DRAM<SMLA_CIO>* node, Command cmd, int id) {
        switch (int(node->state)) {
            case int(State::Closed): return false;
...
void SMLA_CIO::init_lambda()
{
    lambda[int(Level::Bank)][int(Command::ACT)] = [] (DRAM<SMLA_CIO>* node, int id) {
        node->state = State::Opened;
        node->row_state[id] = State::Opened;};
    lambda[int(Level::Bank)][int(Command::PRE)] = [] (DRAM<SMLA_CIO>* node, int id) {
        node->state = State::Closed;
        node->row_state.clear();};
    lambda[int(Level::Rank)][int(Command::PRA)] = [] (DRAM<SMLA_CIO>* node, int id) {
        for (auto bank : node->children) {
            bank->state = State::Closed;
            bank->row_state.clear();}};
    lambda[int(Level::Rank)][int(Command::REF)] = [] (DRAM<SMLA_CIO>* node, int id) {};
    lambda[int(Level::Bank)][int(Command::RD)] = [] (DRAM<SMLA_CIO>* node, int id) {};
    lambda[int(Level::Bank)][int(Command::WR)] = [] (DRAM<SMLA_CIO>* node, int id) {};
    lambda[int(Level::Bank)][int(Command::RDA)] = [] (DRAM<SMLA_CIO>* node, int id) {
        node->state = State::Closed;
        node->row_state.clear();};
    lambda[int(Level::Bank)][int(Command::WRA)] = [] (DRAM<SMLA_CIO>* node, int id) {
        node->state = State::Closed;
        node->row_state.clear();};
    lambda[int(Level::Rank)][int(Command::PD)] = [] (DRAM<SMLA_CIO>* node, int id) {
        for (auto bank : node->children) {
            if (bank->state == State::Closed)
                continue;
    ...
    lambda[int(Level::Rank)][int(Command::PDX)] = [] (DRAM<SMLA_CIO>* node, int id) {
        node->state = State::PowerUp;};
    lambda[int(Level::Rank)][int(Command::SREF)] = [] (DRAM<SMLA_CIO>* node, int id) {
        node->state = State::SelfRefresh;};
    lambda[int(Level::Rank)][int(Command::SREFX)] = [] (DRAM<SMLA_CIO>* node, int id) {
        node->state = State::PowerUp;};
}


void SMLA_CIO::init_timing()
{
